...
field_colors = {
    field: config['SectorColorPalette'][field]
    for field in config['SectorColorPalette']
}
exchange_colors = {
    exchange: config['ExchangeColorPalette'][exchange]
    for exchange in config['ExchangeColorPalette']
}
exchange_colors_bar = {
    exchange.title(): config['ExchangeColorPaletteBar'][exchange]
    for exchange in config['ExchangeColorPaletteBar']
}

# Display names for the figures (for the legend)
display_names = {
    name: config['DisplayNameEch'][name]
    for name in config['DisplayNameEch']
}
...
def build_stacked_bar_chart(arguments: list,
    ...
    json_data = dbs.get_data_between_two_dates("DonneesNationales",
                                               starting_date, ending_date)
    data = dbs.transform_data_to_df(json_data)
    data = dbs.convert_to_numeric(
        data, arguments
    )  # Convert columns to numeric (because some values are strings)
    ...
    data_melted = data_renamed.melt(
        id_vars='date_heure',
        value_vars=list(display_names.values()),
        var_name='Pays',
        value_name='Echange (MW)')  # Melt the DataFrame

    data_melted = data_melted[data_melted['Echange (MW)']
                              != 0]  # Remove rows with 0 value
    ...
    fig = px.bar(data_melted,
                 ...
                 custom_data=['Pays'],
                 title="Echanges d'électricité avec les pays voisins")
...
def build_boxplot_echanges(starting_date: str = default_start_date,
    ...
    fig = px.box(
        ...
        color_discrete_map=exchange_colors_bar,
        title=
        "Boîte à moustaches des échanges d'électricité avec les pays voisins de la France"
    )
...
def build_donuts_exchanges(starting_date: str = default_start_date,
                           ending_date: str = default_end_date):
    ...
    import_colors = [
        exchange_colors.get(label, '#FFFFFF') for label in imports.index
    ]
    export_colors = [
        exchange_colors.get(label, '#FFFFFF') for label in exports.index
    ]
    ...
    if not imports.empty:
        fig.add_trace(
            go.Pie(labels=import_labels,
                   values=imports,
                   marker=dict(colors=import_colors_transparent,
                               line=dict(color=import_colors, width=2)),
                   name='Imports',
                   hole=.4,
                   textfont=dict(color='#FFFFFF')), 1, 1)
    if not exports.empty:
        fig.add_trace(
            go.Pie(labels=export_labels,
                   values=exports,
                   marker=dict(colors=export_colors_transparent,
                               line=dict(color=export_colors, width=2)),
                   name='Exports',
                   hole=.4,
                   textfont=dict(color='#FFFFFF')), 1, 2)
...
def build_stacked_area_chart(argument: str = "nucleaire",
    ...
    data = dbs.get_data_between_two_dates("DonneesRegionales", starting_date,
                                          ending_date)
    fig = px.area(data,
                  x="date_heure",
                  y=str(argument),
                  color="libelle_region",
                  title=f"Evolution de la production {argument} par région",
                  template="plotly_dark")
...
def build_stacked_area_by_production_region(
        region: str,
        starting_date: str = default_start_date,
    ...
    plotly.graph_objects.Figure
        Figure containing the stacked area chart."""
    data = dbs.transform_data_to_df(
        dbs.get_data_between_two_dates("DonneesRegionales",
                                       starting_date,
                                       ending_date,
                                       region=region))
    ...
    production_fields = [
        "eolien", "hydraulique", "nucleaire", "solaire", "thermique"
    ]
    if not set(production_fields).issubset(data.columns):
        missing_fields = list(set(production_fields) - set(data.columns))
        raise ValueError(
            f"Les colonnes suivantes sont manquantes dans le DataFrame: {missing_fields}"
        )

    fig = px.area(data,
                  x="date_heure",
                  y=production_fields,
                  title="Production par filière",
...
def build_stacked_area_two_regions(
        region1: str,
        region2: str,
        starting_date: str = default_start_date,
        ending_date: str = default_end_date) -> px.area:
    ...
    plotly.graph_objects.Figure
        Figure containing the stacked area chart."""
    data = dbs.transform_data_to_df(
        dbs.get_data_between_two_dates("DonneesRegionales", starting_date,
                                       ending_date))
    data = data.sort_values(by=['date_heure'])
    data = dbs.remove_nan_from_data(data, "consommation")
    data = data[data["libelle_region"].isin([region1, region2
                                             ])]  # Keep only the two regions

    production_fields = [
        "eolien", "hydraulique", "nucleaire", "solaire", "thermique"
    ]
    # Sum the production fields for each region by date_heure to make one total production column for each region
    data = data.groupby(["date_heure", "libelle_region"
                         ])[production_fields].sum().reset_index()
...
def build_line_chart_with_prediction(starting_date: str = default_start_date,
    ...
    # Check if the necessary columns are in the DataFrame
    if not {'date_heure', 'consommation', 'prevision_j', 'prevision_j1'
    ...
    fig.update_layout(title="Consommation nationale avec prédictions",
                      template="plotly_dark",
                      hovermode="x unified")
...
def build_line_chart_consommation_by_region(
        starting_date: str = default_start_date,
        ending_date: str = default_end_date,
    ...
    """
    # Fetching data and converting JSON to DataFrame
    json_data = dbs.get_data_between_two_dates("DonneesRegionales",
                                               starting_date, ending_date)
    data = dbs.transform_data_to_df(json_data)
    #Order data by datetime
    data = data.sort_values(by=['date_heure'])
    ...
    # Check if the necessary columns are in the DataFrame
    if not {'date_heure', 'consommation', 'libelle_region'}.issubset(
            data.columns):
        raise ValueError(
            "One or more required columns are missing in the data")
    ...
    line_chart_cons.update_traces(
        hovertemplate="%{y:.2f} MW<br>Date: %{x|%Y-%m-%d %H:%M}")

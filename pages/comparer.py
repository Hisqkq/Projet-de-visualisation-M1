...
def layout(region1, region2):
    ...
    return dbc.Container([
        dbc.NavbarSimple(
            ...
            children=[
                dcc.Link(dbc.Button(
                    'Retour', color="light", className="ms-auto"),
                         href=dash.page_registry['pages.production']['path']),
                dcc.Link(dbc.Button(
                    'Accueil', color="light", className="ms-auto"),
                         href='/')
            ],
            style={
                "fontSize": "1.5rem",
        ...
        dbc.Row([
            dbc.Col([
                dcc.Graph(id="production_pie_chart_by_sector_1",
                          figure=pie_chart.
                          regional_pie_chart_production_by_sector(region1),
                          config={'displayModeBar': False})
            ],
                    width=6),
            dbc.Col([
                dcc.Graph(id="production_pie_chart_by_sector_2",
                          figure=pie_chart.
                          regional_pie_chart_production_by_sector(region2),
                          config={'displayModeBar': False})
            ],
                    width=6)
        ]),
        dbc.Row([
            dbc.Col([
                dcc.Graph(id="stacked_area_chart_by_region",
                          figure=figures.build_stacked_area_two_regions(
                              region1, region2),
                          config={'displayModeBar': False})
            ]),
        ]),
...
def update_graphs(pathname, dates):
    ...
    if match:
        ...
        pie_chart_1 = pie_chart.regional_pie_chart_production_by_sector(
            region1, starting_date, ending_date)
        pie_chart_2 = pie_chart.regional_pie_chart_production_by_sector(
            region2, starting_date, ending_date)
        stacked_area_chart = figures.build_stacked_area_two_regions(
            region1, region2, starting_date, ending_date)

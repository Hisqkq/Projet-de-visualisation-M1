...
def layout():
    return dbc.Container([
        dbc.NavbarSimple(brand="Les échanges commerciaux aux frontières",
                         color="primary",
                         dark=True,
                         className="mb-4",
                         children=[
        ...
        dbc.Row([
            dbc.Col(dcc.Graph(id='donut_chart_echanges',
                              figure=figures.build_donuts_exchanges(
...
def update_bar_chart_echanges(dates):
    ...
    return figures.build_stacked_bar_chart(arguments=[
...
def update_boxplot_echanges(dates):
    ...
    if dates is None:
        dates = [default_start_date, default_start_date]
    return figures.build_boxplot_echanges(starting_date=dates[0],
                                          ending_date=dates[1])
...
def update_donut_chart_echanges(dates):
    ...
    if dates is None:
        dates = [default_start_date, default_start_date]
    return figures.build_donuts_exchanges(starting_date=dates[0],
                                          ending_date=dates[1])
